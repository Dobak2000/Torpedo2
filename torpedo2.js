...
function step1() {
	document.getElementById("textBox").innerHTML = '<p>Helyezd el az 5 hajódat!</p><p>Kattints a kiválasztott mezőkre az 1. táblán!</p>';
...
function placeBoat(id) {
	if (tdClick) {
		if (myBoats.length < 5) {
			console.log(myBoats.length);
			console.log(myBoats);
			if (myBoats.indexOf(id) == -1) {
...
function step2() {
	...
	document.getElementById("mainButton").children[0].disabled = "disabled";
	document.getElementById("boatsLeft").children[0].innerHTML = "Lelövendő hajók száma " + enemyBoatsLeft + " - " + myBoatsLeft;
...
function shootBoat(id) {
	if (shooting) {
		if (enemyBoatsId.indexOf(id) > -1) {
			enemyBoatsLeft--;
			myGuesses.push(id);

			var index = enemyBoatsId.indexOf(id);
			enemyBoatsId.splice(index, 1);

			console.log(enemyBoatsId);
			...
			console.log("Hé, erre már kattintottál!");
		} else {
...
function gameGoing() {
	while (playing) {
		...
		if (myBoats.indexOf(compGuessId) > -1) {
			myBoatsLeft--;

			var index = myBoats.indexOf(compGuessId);
...
function newGame() {
	...
	document.getElementById("result").innerHTML = '<p>Eredmény: ' + myPoints + ' - ' + compPoints + '</p>';
	document.getElementById("boatsLeft").children[0].innerHTML = "";
}